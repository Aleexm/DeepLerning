...
def plotClassAccuracy(df1, df2, label):
    ...
    w=0.2
    plt.bar(np.arange(0,43,1)-1.5*w, origAccGer, width=w, label = "Orig Eur", color = 'blue')
    plt.bar(np.arange(0,43,1)-0.5*w, origAccEur, width=w, label = "Orig Aug", color = 'red')
    plt.bar(np.arange(0,43,1)+0.5*w, accuracies1, width=w, label = "{} Eur".format(label), color = 'orange')
    plt.bar(np.arange(0,43,1)+1.5*w, accuracies2, width=w, label = "{} Aug".format(label), color = 'g')
